import uuid
from datetime import datetime
from typing import Annotated, List, Optional

from fastapi import APIRouter, Depends, HTTPException, Query
from sqlmodel import Session, select

from app.database import get_session
from app.models import Movie, MovieStatus
from app.schemas import MovieCreate, MovieRead, RateRequest

router = APIRouter(prefix="/movies", tags=["movies"])

SessionDep = Annotated[Session, Depends(get_session)]


@router.post("", response_model=MovieRead, status_code=201)
def add_movie(body: MovieCreate, session: SessionDep) -> Movie:
    movie = Movie(title=body.title, year=body.year)
    session.add(movie)
    session.commit()
    session.refresh(movie)
    return movie


@router.get("", response_model=List[MovieRead])
def list_movies(
    session: SessionDep,
    status: Optional[MovieStatus] = Query(None),
) -> List[Movie]:
    query = select(Movie)
    if status is not None:
        query = query.where(Movie.status == status)
    return list(session.exec(query).all())


@router.patch("/{movie_id}/watch", response_model=MovieRead)
def mark_watched(movie_id: uuid.UUID, session: SessionDep) -> Movie:
    movie = session.get(Movie, movie_id)
    if movie is None:
        raise HTTPException(status_code=404, detail="Movie not found")
    movie.status = MovieStatus.watched
    movie.watched_at = datetime.utcnow()
    session.add(movie)
    session.commit()
    session.refresh(movie)
    return movie


@router.patch("/{movie_id}/rate", response_model=MovieRead)
def rate_movie(movie_id: uuid.UUID, body: RateRequest, session: SessionDep) -> Movie:
    if not 1 <= body.rating <= 10:
        raise HTTPException(status_code=422, detail="Rating must be between 1 and 10")
    movie = session.get(Movie, movie_id)
    if movie is None:
        raise HTTPException(status_code=404, detail="Movie not found")
    movie.rating = body.rating
    movie.review = body.review
    session.add(movie)
    session.commit()
    session.refresh(movie)
    return movie


@router.delete("/{movie_id}", status_code=204)
def delete_movie(movie_id: uuid.UUID, session: SessionDep) -> None:
    movie = session.get(Movie, movie_id)
    if movie is None:
        raise HTTPException(status_code=404, detail="Movie not found")
    session.delete(movie)
    session.commit()
