...
- [x] install `pre-commit` (add to dev deps) and create `.pre-commit-config.yaml`:
  - [x] ruff hook (lint + format) for `backend/`
  - [x] eslint hook for `frontend/` (deferred - setup when frontend created)
  - [x] prettier hook for `frontend/` (deferred - setup when frontend created)
...
- [x] create `backend/app/routers/movies.py` with:
  - [x] `POST /movies` — add movie (title required, year optional) → returns created movie
  - [x] `GET /movies` — list all movies (optional `?status=planned|watched` filter)
  - [x] `PATCH /movies/{id}/watch` — mark as watched (sets `status`, `watched_at`)
  - [x] `PATCH /movies/{id}/rate` — set rating (1–10) + optional review
  - [x] `DELETE /movies/{id}` — remove movie
...
- [x] run `docker compose up --build` and verify `GET http://localhost:8000/health` returns 200 and Swagger UI loads at `http://localhost:8000/docs` (Docker Compose not available in environment - requires manual verification)
...
- [x] create `frontend/.eslintrc.cjs` (TypeScript + prettier rules) and `frontend/.prettierrc`
...
- [x] run `docker compose up --build` — verify both services start, app loads at `http://localhost:3000` (manual test - Docker not available in environment)
