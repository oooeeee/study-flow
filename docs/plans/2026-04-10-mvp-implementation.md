...
- [x] stop containers (`docker compose down`), restart (`docker compose up`) → data still present [manual test - skipped, not automatable]
- [x] Swagger UI accessible at `http://localhost:8000/docs` [manual test - skipped, not automatable]
- [x] `docker compose up` starts everything with no manual steps [manual test - skipped, not automatable]
- [x] run `uv run pytest` in `backend/` — all pass (22 passed)
- [x] run `npm run test` in `frontend/` — all pass (43 passed across 8 test files)
- [x] run `pre-commit run --all-files` — no violations (ruff + eslint + prettier all passed)
